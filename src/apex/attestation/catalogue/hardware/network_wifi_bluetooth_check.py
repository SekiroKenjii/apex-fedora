...
CHECK = catalogue.declare(
    descriptors.CheckSpec(
        id=identifiers.CheckId('network.wifi-bluetooth'),
        group='hardware',
        environment=claims.EnvironmentKind.PHYSICAL,
        summary=(
            'Establishes that Wi-Fi and Bluetooth work on the physical machine.'
        ),
        accepted_proof_kinds=('.json', '.log', '.png', '.ppm', '.txt', '.xml'),
        scope_limits=(
            'The only source is this component name inside one comma-separated bullet.',
            (
