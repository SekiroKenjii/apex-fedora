"""Sharing identical extents between two files, as the kernel is asked to do it.

The request and reply layouts are the Linux x86_64 ones the older script packed by hand;
they are values here so a port can carry them and a fake can answer them. A range must be
positive and block aligned, which is the rule the kernel applies and the one a caller learns
about first.
"""
