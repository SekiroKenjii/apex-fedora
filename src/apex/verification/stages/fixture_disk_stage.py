...
def script(remote: safepaths.RemotePath, frozen: oci.FrozenImage) -> guestshell.RemoteScript:
    inner = guestshell.RemoteScript.of(
        guestshell.Step.of(
            "bash",
            f"guest/{builds.ArtifactKind.QCOW2.guest_script}",
            str(builds.ArtifactKind.QCOW2),
            str(frozen.image_id),
        ),
...
STAGE = stages.SimpleStage(
    ...
    reads=(
        composition_keys.BUILDER_VERIFIED,
        composition_keys.REMOTE,
        composition_keys.FROZEN,
        composition_keys.ACCESS,
        verifykeys.TAGGED,
