...
A name the registry holds runs as a command with the context the root built; every other
name goes to the bridge, so the older tree keeps answering for what has not moved. A refusal
raised by a command is rendered with its own exit code, and a parser that stops the run,
for help or a wrong operand, keeps the code it chose.
"""
