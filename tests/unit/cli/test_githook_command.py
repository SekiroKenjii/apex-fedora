...
def test_a_refused_message_comes_back_as_the_rules_text_with_their_exit_code(
    ...
    reply = githook_command.run(
        commandspecs.Request(arguments=("commit-msg", str(message)), context=context(processes))
    )
...
def test_pre_push_reads_standard_input_and_the_other_kinds_leave_it_alone(tmp_path: Path) -> None:
    reads: list[str] = []
    ...
    pushed = githook_command.run(
        commandspecs.Request(
            arguments=("pre-push", "origin", "url"),
            context=context(processes),
            read_input=read_input,
        )
    )
    committed = githook_command.run(
        commandspecs.Request(
            arguments=("commit-msg", str(message)),
            context=context(processes),
            read_input=read_input,
        )
    )
...
def test_a_context_without_repository_ports_cannot_answer_a_hook(tmp_path: Path) -> None:
    ...
    with pytest.raises(errors.PreconditionUnmet) as refused:
        githook_command.run(
            commandspecs.Request(arguments=("commit-msg", str(message)), context=context(None))
        )
...
def test_a_git_failure_is_the_reply_s_narrative_not_a_traceback() -> None:
    ...
    reply = githook_command.run(
        commandspecs.Request(arguments=("pre-commit",), context=context(processes))
    )
