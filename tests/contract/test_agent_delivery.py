...
def test_the_unpacked_agent_runs_a_unit_and_frames_the_reply(library: Path) -> None:
    request = json.dumps({
        "protocol": agentwire.PROTOCOL_VERSION, "host_version": "source",
        "unit": "guest.state", "arguments": {}, "agent_digest": "a" * 64,
    }).encode()

    completed = subprocess.run(
        [sys.executable, "-m", "apex.agent.main", "run", "--framed", str(TOKEN)],
        input=request, env={"PYTHONPATH": str(library), "PATH": "/usr/bin:/bin"},
