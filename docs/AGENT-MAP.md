...
|---|---|---|---|---|
| `guest/probe.py` | 22 | probe | `guest.state` unit; the older script was deleted in P21 | P16, done |
...
| `guest/recovery-probe.py` | 68 | probe | `recovery.prerequisites` unit, asked by `test-update check-a`; the older script was deleted in P21 | P19d, done |
| `guest/installed-recovery-probe.py` | 58 | probe | `recovery.installed` unit with `agent/grubstatic.py`, asked by `test-recovery verify-installed`; the older script was deleted in P21 | P19d, done |
| `guest/render-probe.py` | 37 | probe, GTK | `assets/verbatim/render-probe.py.verbatim` is the one copy, its digest held by the wrapper; placed and started by the `desktop.render` unit as the session's user, host case in `verification/probes/desktop_render_probe.py`; the older program was deleted in P21 | verbatim, P19g, done |
| `guest/theme-probe.py` | 54 | probe, GTK | `assets/verbatim/theme-probe.py.verbatim` is the one copy, its digest held by the wrapper; shown and stopped by the `desktop.theme-gtk3` and `desktop.theme-adwaita` units, the session's settings read by `desktop.theme-settings`; host cases under `verification/probes/`; the older program was deleted in P21 | verbatim, P19g, done |
...
| `guest/initramfs-fixture.py` | 260 | fixture | `fixture.initramfs` unit, the entries bound by `agent/bootentries.py`, the pure rules in `provisioning/fixtures/initramfs_fixture.py`; asked by `test-initramfs-inspect`, `test-initramfs-inject` and `test-initramfs-rescue` through the account's password in a private mount namespace; the older script was deleted in P21 | P21, done |
| `guest/recovery-fixture.py` | 182 | fixture | `fixture.recovery` unit, the pure derivations in `provisioning/fixtures/recovery_fixture.py`; asked by `test-recovery repair-grub`, `arm-gdm` and `retry-config` in a private mount namespace; the older script was deleted in P21 | P21, done |
| `guest/update-fixture.py` | 188 | fixture, signing | `fixture.update` unit on the container engine, host side in `provisioning/fixtures/update_fixture.py`, asked by `update-fixtures`; the older script was deleted in P21 | P18c, done |
...
| `guest/dedupe-update-blobs.py` | 153 | fixture, storage | `fixture.dedupe` unit over the extent port, layout in `model/extents.py`, asked by `just dedupe`; the older script was deleted in P21 | P18c, done |
